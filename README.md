# Hi, I'm Darrell Gayok Insor 👋

I am a finance graduate interested in financial analysis, ESG research and business intelligence. I am building practical projects using Power BI, SQL, Excel and Power Query.

## About Me

- 🎓 Bachelor of Finance graduate from Universiti Malaysia Sarawak, Class of 2026
- 📊 Interested in financial analysis, ESG and business performance
- 🌱 Developing my Power BI, SQL and Power Query skills
- 📍 Based in Kuching, Sarawak, Malaysia
- 💼 Open to graduate opportunities in finance, accounting and data analysis

## Tools and Skills

- Excel
- Power Query
- Power BI
- SQL
- Financial analysis
- ESG research
- STATA

## Featured Projects

### [Sales Funnel Analysis](https://github.com/darrellgayok/sales-funnel-analysis)

SQL project analysing user activity, sales funnel performance, traffic sources, conversion rates, time to conversion, and revenue metrics.

- Tools: SQLite, DBeaver, DB Browser for SQLite and GitHub
- Focus: Funnel stages, user behaviour, traffic-source performance and revenue analysis
- Key skills: `COUNT`, `CASE WHEN`, `GROUP BY`, CTEs, date filtering, conversion-rate calculations and revenue metrics
- Status: Completed first portfolio project

### ESG and Firm Performance Dashboard

Power BI project based on my undergraduate research into ESG, green finance and firm performance.

- Tools: Power BI, Excel and Power Query
- Focus: ESG performance, firm performance indicators and dashboard storytelling
- Status: Dashboard development in progress

## Connect With Me

- LinkedIn: [linkedin.com/in/darrell-gayok-insor](https://www.linkedin.com/in/darrell-gayok-insor/)
- Email: [darrellgayok@gmail.com](mailto:darrellgayok@gmail.com)
